...
Solo and multiplayer-lobby creation previously selected the first eligible published round, so repeated plays
could return the same route indefinitely. Multiplayer alone had a short lobby countdown, while Solo and
...
Every newly created session receives an absolute `started_at` five seconds in the future. Multiplayer
...
Solo players and lobby hosts confirm Easy, Normal, or Hard before creation. Daily remains curated.
...
exists. Automatic Solo, multiplayer-lobby, and new Daily assignments share one graph-derived eligibility
...
- Controls, scoring time, and multiplayer fairness share one server-owned boundary.
- Category and endpoint presentation is consistent in Solo, Daily, and Multiplayer without making the
...
- Solo, multiplayer-lobby, and new Daily assignments share one source-independent round-eligibility rule.
