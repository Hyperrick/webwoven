...
def test_starter_seeds_cover_every_category_with_real_qids() -> None:
    ...
    assert len(catalog.seeds) == 200
    assert all(len(grouped[category]) == 20 for category in CATEGORIES)
...
def test_seed_provenance_manifest_matches_reviewed_catalog() -> None:
    manifest_path = SEEDS_PATH.parents[1] / "manifests/seed-catalog.v2.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))

    assert manifest["qid_count"] == 200
